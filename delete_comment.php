...
if (isset($_SERVER["REQUEST_METHOD"]) && $_SERVER["REQUEST_METHOD"] == "POST") {
    if ($_POST["submit"] == "Xóa") {
        $sql = "DELETE FROM nguoidungdanhgiasp WHERE (rUsername = '$uid' && rProductId = '$id' )";
        if ($conn->query($sql) === TRUE) {
            echo '<script>alert("Đã xóa thành công!")</script>';
            echo '<script>location.href = "comment_admin.php" </script>';
        }
    } 
}
?>
...
<body style="background-color:black;">
    <div class="container mt-3" style="width: 600px;">
        ...
        <form method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]) . '?id=' . $id .'&uid=' . $uid ?>">
            ...
            <a class="form-control btn btn-dark mt-3" href="comment_admin.php>">Hủy</a><br>
        </form>
    </div>
